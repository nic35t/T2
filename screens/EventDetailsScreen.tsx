...
export const EventDetailsScreen: React.FC<EventDetailsScreenProps> = ({ event, onNavigate, onBack }) => {
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 dark:bg-background text-gray-900 dark:text-white animate-fade-in font-sans transition-colors duration-300">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex h-auto min-h-[64px] items-center justify-between px-4 pt-[env(safe-area-inset-top,16px)] pb-2 lg:px-10 bg-gradient-to-b from-black/60 to-transparent pointer-events-none">
        <button 
          onClick={onBack}
      ...
      <div className="flex flex-col lg:flex-row min-h-screen">
        ...
        <div className="relative h-[60vh] md:h-[50vh] lg:h-screen lg:w-1/2 lg:sticky lg:top-0 shrink-0">
          ...
          ></div>
          {/* Gradient always dark at bottom to support white text */}
        ...
        <div className="lg:w-1/2 lg:bg-gray-50/50 dark:lg:bg-background/40 flex flex-col bg-gray-50 dark:bg-background">
          {/* Tabs */}
          <div className="sticky top-[env(safe-area-inset-top,64px)] z-40 bg-white/95 dark:bg-background/95 backdrop-blur-xl border-b border-gray-200 dark:border-white/10 flex px-6 transition-colors">
             <button 
             ...
             <button 
               ...
               className={`flex-1 py-4 text-sm font-bold uppercase tracking-widest relative transition-colors ${activeTab === 'reviews' ? 'text-primary' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
             >
                Reviews (24)
          ...
          <div className="px-6 py-10 lg:p-20 space-y-12 flex-1 pb-32">
            ...
            {activeTab === 'info' ? (
              <div className="animate-fade-in space-y-12">
                ...
                <div className="grid grid-cols-3 gap-4 md:gap-8 lg:gap-12 border-b border-gray-200 dark:border-white/10 pb-8">
                  <div className="flex flex-col gap-1.5">
                    <span className="text-[10px] lg:text-xs text-gray-500 uppercase tracking-widest font-bold font-sans">Date</span>
                    <span className="text-sm lg:text-lg font-medium font-mono tracking-tight text-gray-900 dark:text-white">{event.date}</span>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <span className="text-[10px] lg:text-xs text-gray-500 uppercase tracking-widest font-bold font-sans">Time</span>
                    <span className="text-sm lg:text-lg font-medium font-mono tracking-tight text-gray-900 dark:text-white">{event.time}</span>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <span className="text-[10px] lg:text-xs text-gray-500 uppercase tracking-widest font-bold font-sans">Runtime</span>
                    <span className="text-sm lg:text-lg font-medium font-mono tracking-tight text-gray-900 dark:text-white">160min</span>
                  </div>
                </div>
                  ...
                  <div className="flex gap-6 lg:gap-10 overflow-x-auto no-scrollbar pb-4">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div key={i} className="flex flex-col items-center gap-3 shrink-0 group cursor-pointer">
                        <div className="size-20 lg:size-24 rounded-full bg-gray-200 dark:bg-white/5 border border-gray-300 dark:border-white/10 overflow-hidden group-hover:border-primary transition-all shadow-md dark:shadow-lg">
                        ...
                        <span className="text-[11px] lg:text-xs text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white font-medium">Artist</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="animate-fade-in space-y-6">
                 {/* Reviews Summary */}
                 <div className="flex items-center gap-4 bg-white dark:bg-surface-card p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm">
                    <div className="text-center">
                       <span className="block text-3xl font-bold font-serif text-gray-900 dark:text-white">4.8</span>
                       <span className="text-[10px] text-gray-500 uppercase tracking-wide">Out of 5.0</span>
                    </div>
                    <div className="flex-1 h-12 border-l border-gray-200 dark:border-white/10 pl-6 flex flex-col justify-center">
                       <div className="flex text-primary mb-1">
                          {[1,2,3,4,5].map(star => <span key={star} className="material-symbols-outlined text-lg font-filled">star</span>)}
                       </div>
                       <p className="text-xs text-gray-500">Based on 24 verified reviews</p>
                    </div>
                 </div>
                 ...
                 <div className="space-y-4">
                    {MOCK_REVIEWS.map((review) => (
                       <div key={review.id} className="p-4 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/5 shadow-sm">
                          <div className="flex justify-between items-start mb-2">
                             <div className="flex items-center gap-2">
                                <div className="size-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs font-bold text-gray-700 dark:text-white">
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed pl-10">
                             {review.content}
                          </p>
                       </div>
                    ))}
      ...
      {/* Sticky Footer */}
      <div className="fixed bottom-0 left-0 right-0 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] lg:p-6 bg-white/90 dark:bg-background/90 backdrop-blur-xl border-t border-gray-200 dark:border-white/10 z-40 transition-colors">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
          ...
          <button 
            ...
            className="flex-1 max-w-xl h-14 lg:h-16 bg-gray-900 dark:bg-white/10 backdrop-blur-md border border-transparent dark:border-white/20 text-white text-sm lg:text-base font-bold uppercase tracking-widest rounded-2xl shadow-xl dark:shadow-glow hover:bg-black dark:hover:bg-white/20 active:scale-[0.98] transition-all"
          >
            Select Seat
          </button>
        </div>
