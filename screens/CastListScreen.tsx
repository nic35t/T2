...
export const CastListScreen: React.FC<CastListScreenProps> = ({ event, onBack }) => {
  ...
  return (
    <div className="relative min-h-screen bg-background text-white pb-20 animate-fade-in font-sans overflow-hidden">
      {/* Texture Background */}
      <div 
      ...
      <header className="sticky top-0 left-0 right-0 z-50 flex h-16 items-center px-4 bg-background/80 backdrop-blur-xl border-b border-white/5">
      ...
      <main className="max-w-7xl mx-auto p-6 lg:p-12 relative z-10">
        <div className="mb-12 lg:text-center">
          ...
          <h2 className="text-3xl md:text-5xl font-serif font-bold">Leading Artists</h2>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 lg:gap-10">
          {castMembers.map((member, idx) => (
            <div 
              key={member.name} 
              className="bg-surface-card/50 backdrop-blur-md border border-white/5 rounded-[32px] p-6 flex flex-col items-center text-center animate-fade-in-up hover:border-primary/20 transition-all group"
              style={{ animationDelay: `${idx * 0.05}s` }}
            >
              <div className="size-24 lg:size-32 rounded-full overflow-hidden border-2 border-primary/20 mb-5 shadow-2xl group-hover:border-primary transition-all duration-500">
                <img src={member.image} alt={member.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
              </div>
              <h3 className="text-base lg:text-lg font-bold text-white mb-1">{member.name}</h3>
        ...
        <div className="mt-20 pt-12 border-t border-white/10 max-w-3xl mx-auto">
          <h3 className="text-xl lg:text-3xl font-serif font-bold mb-8 lg:text-center">Production Team</h3>
          <div className="space-y-6">
            {[
              ...
              <div key={crew.role} className="flex justify-between items-center py-4 border-b border-white/5 last:border-0 hover:bg-white/[0.02] px-4 rounded-xl transition-all">
                <span className="text-xs lg:text-sm text-gray-400 font-bold uppercase tracking-widest">{crew.role}</span>
                <span className="text-base lg:text-xl font-bold text-white">{crew.name}</span>
              </div>
            ))}
          </div>
        </div>
